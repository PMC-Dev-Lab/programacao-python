...
def adicionar_page():
    ...
    MAX_NOME_LENGTH = 100          # *** NOVO: Definição do limite máximo para o nome ***
    MAX_DESCRICAO_LENGTH = 500     # *** NOVO: Definição do limite máximo para a descrição ***
    MAX_IMAGE_SIZE_BYTES = 2 * 1024 * 1024 # *** NOVO: Definição do limite máximo para o tamanho da imagem (2MB) ***
    ...
    with st.form(key='produto_form_adicionar'):
        ...
        if submit_button:
            if not nome.strip():
            ...
            elif len(nome) > MAX_NOME_LENGTH: # *** NOVO: Validação do comprimento do nome ***
                st.error(f'O nome do produto não pode ter mais de {MAX_NOME_LENGTH} caracteres.') # *** NOVO: Mensagem de erro clara ***
            elif nome.isdigit():
            ...
            elif len(descricao) > MAX_DESCRICAO_LENGTH: # *** NOVO: Validação do comprimento da descrição ***
                st.error(f'A descrição do produto não pode ter mais de {MAX_DESCRICAO_LENGTH} caracteres.') # *** NOVO: Mensagem de erro clara ***
                ...
                if imagem_file is not None:
                    if imagem_file.size > MAX_IMAGE_SIZE_BYTES: # *** NOVO: Validação do tamanho da imagem ***
                        st.error(f"A imagem é demasiado grande. O tamanho máximo permitido é {MAX_IMAGE_SIZE_BYTES/(1024*1024):.2f} MB.") # *** NOVO: Mensagem de erro clara sobre o tamanho da imagem ***
