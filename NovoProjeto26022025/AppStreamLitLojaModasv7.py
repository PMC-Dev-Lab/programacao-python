...
# Sidebar Menu
abas = ["Home", "Adicionar Produto", "Mostrar Produtos", "Editar Produto", "Eliminar Produto"]
aba_selecionada = st.sidebar.radio("Escolha uma opção", abas)

# Placeholder functions to show different pages
def home_page():
    st.subheader("Bem-vindo à Loja de Moda!")
    
    st.write("Esta é uma plataforma para gerenciar os produtos da loja.")
...
def add_product_page():
    ...
    with st.form(key='produto_form_adicionar'):
        ...
        if imagem_file is not None:
            imagem_nome = imagem_file.name  # Sanitiza o nome da imagem
            imagem = f"images/{imagem_nome}"
        ...
        if submit_button:
            ...
            if not nome.strip():  # Verifica se o nome está vazio ou contém apenas espaços
                ...
                st.error('A descrição não pode ter conter simbolos.')
            elif preco <= 0:
                st.error('O preço deve ser maior que zero.')
            else:
                # Se a imagem for fornecida, usamos a imagem carregada, caso contrário, usamos a imagem padrão
                if imagem:
...
def delete_product_page():
    ...
    st.write("Aqui você pode eliminar um produto existente.")

# Show corresponding page based on sidebar selection
