...
def login_page():
    ...
    if st.session_state["logged_in"]:
        with st.container(): # *** NOVO: Container para a área de perfil ***
            ...
            # *** CAMPOS EDITÁVEIS PARA NOME E EMAIL ***
            ...
            if st.button("Guardar Alterações", key="btn_guardar_perfil"):
                # *** Lógica para guardar as alterações do perfil ***
                ...
                if atualizar_utilizador(user_id, nome, email):
                    ...
                    # Atualizar informações na sessão para refletir as alterações
                    st.session_state['user_nome'] = nome
                    st.session_state['user_email'] = email
                else:
                    st.error("Erro ao atualizar o perfil. Tente novamente.")

            st.markdown(f"**Username:** {st.session_state['username']}") # Username continua a ser apenas exibido
